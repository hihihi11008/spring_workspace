...
public class ExcelManager {
	FileInputStream fis;
	public ExcelManager() {
		//로컬의 파일을 접근하기 위해서는 파일 스트림이 필요하다 
		try {
			...
			//파일접근은 성공한 상태이므로, 이시점부터는 쉬트에 접근한다 
			XSSFSheet sheet = workbook.getSheetAt(0);//첫번째 sheet선택
			
			//이 시점부터는 sheet를 제어할 수 있으므로, 총 몇건이 존재하느지부터 알아본다 
			int rows=sheet.getPhysicalNumberOfRows();//데이터가 채워진 row 수
			System.out.println("레코드 수는 : "+rows);
			
			//로우와 컬럼수만큼 반복문실행하여 데이터를 제어해보자 
			for (int i=0; i<rows;i++) {
				...
				for(int a =0; a<columnCount;a++) {
					...
					if (cell.getCellType() == CellType.STRING) {//셀의자료형이 문자인경우
